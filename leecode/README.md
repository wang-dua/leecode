# 704 二分查找: 
1. 遵循循环不变量原则, 双闭区间, 半开半闭区分清楚
2. 无论while是那种条件, 更新low一定是+1
3. while(<=), 这样mid也会被判断, 所以当mid不符合时, 直接可以跳过, 这样mid一定可以判断不是target, high--

# 59 螺旋矩阵: 
**同样遵循循环一致性原则**, 利用 count 变量 填充, 设定偏移值offset用于循环条件,按圈数(n/2)循环, 四次是一个循环, 每次循环结束后的开始位置要+1, 同时n为奇数时注意最后要填充中间值,
...
# 121 买卖股票的最佳时机(单词买入卖出)
* dp[i][0]: 是i日未持有股票的最大收益
* dp[i][0]: 是i日持有股票的最大收益
* 当第i天持有股票: i-1持有->i持有    i-1未持有->持有(第一次买入)
* 当第i天未持有股票: i-1未持有->i未持有    i-1持有->持有(卖出)
* 注意, 只能买一次, 那么从i-1未持有到i持有后的收益一定是-prices[i], 而不是用收益减去它

# 122 买卖股票的最佳时机(一只股票多次买入卖出)
* 当第i天未持有股票: i-1未持有->i未持有    i-1持有->持有(卖出)
*  **这里的从i-1未持有到i持有后的收益是 用之前的收益减去prices[i], 这是和单词买卖的不同之处**

# 123 一只股票买卖最多两次
状态分别是 不操作/买入/卖出/买入/卖出, 然后推导每一列的dp[i][j] 即可

# 买卖股票总结
1. 其实结合 123题, 121 122应当加上`不操作`一列更好理解, 这样才能体现出只能单词买入和不限次数买入的区别, 首次买入是从不操作一列转移过来的, 应该都用买入 卖出状态表示容易理解
2. 值得注意的是, 买入卖出状态只是当天的状态, 并非当天买入或卖出, 第i天的状态是买入, 可能是前面买入的

# 300 最长递增子序列长度
给定一个数组, 求最长的递增子序列长度, 子序列从数组中抽出的元素的序列, 相对顺序和原数组相同
1. dp[i]: 表示 在nums[i]元素结尾的递增子序列中, 最长的序列长度, nums = [10, 9, 2, 5, 3, 7, 101, 18], 比如nums[5] = 7, 以7结尾的递增子序列有 257, 37, 
2. 推导: 如果nums[i] > nums[0-(i-1)], 说明 nums[i] 可以 和 nums[0-(i-1)]构成递增关系, `dp[i] = max[dp[i], dp[j] + 1]`
3. 初始化: dp[i] = 1, 最短为1
4. 从前向后遍历 

# 674 最长连续递增子序列长度
题300是与前面每个dp[j]比较, 这次只需要比较dp[i] dp[i-1], 
还可以使用贪心法记录count, dp[i] < dp[i-1] count=1, 中断

# 718 最长公共子数组
子数组不是序列, 数从原数组中截取的连续的部分, 不能跨元素
1. dp[i][j] 分别以A[i]B[j]为结尾的两个子数组的最长的连续公共部分
2. 当A[i]B[j]相等的时候, `dp[i][j] = dp[i-1][j-1] + 1`
3. 第一列 第一行初始化
4. 遍历的时候注意要从第一行开始遍历, 因为result要记录长度结果, 从一开始会漏掉结果 比如 1 2 3 和 4 5 1 

# 1143 1035 求最长公共子序列的长度
求子序列跟求数组的最大区别就是, **子序列可以跨元素, 而子数组时原数组的连续的部分**, 在718中, 当元素不等dp[i][j]直接为0, 而这里当元素不同时, dp[i][j]要从dp[i-1][j] 和dp[i][j-1] 继承前面的长度, **因为当前元素不同可以跨越当前的元素**, 可能有些难以理解, 记住即可

# 求子序列子数组总结
1. 300: 求最长递增子序列, 跨元素, 不递增也从前面0-j继承dp
2. 674: 求最长连续递增子序列, 会中断, 不连续就不继承
3. 718: 最长公共子数组, 中断为0
4. 1143 1035: 不中断, 不等的话从上或左继承
...
# 146 LRU
最好使用容器实现, 比较简单不繁琐
1. list<pair<int, int>> 存放LRU队列, 相当于节点链表
2. unordered_map<list<pair<int, int>>::iterator> 用于O(1)查找页面
3.  